<?php
	...
	function my_files_menu() {
		add_submenu_page('m10-remote-ftp-settings', 'M10 Remote FTP', 'My Files', 'read', 'm10-remote-ftp-upload-my-files', 'display_my_files');
	}
	add_action( 'admin_menu', 'my_files_menu' );

	//my files screen
	function display_my_files(){
		...
		//indexing
		if(isset($_GET['m10p'])){$index_start = (15*$_GET['m10p']);}	
		else{$index_start = 0;}
		...
		if($file_count_total>0){
			...
			?>	
				<a class="button grey <?php if($i==$_GET['m10p']){echo 'dark';} ?>" style="color:#f1f1f1;" href="admin.php?page=m10-remote-ftp-upload-my-files&m10p=<?php echo $i; ?>"><?php echo ($i+1); ?></a>
			<?php
			}
			echo "</div>";
